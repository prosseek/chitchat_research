 ...
 * @author teemuk
 */
public class PingApplication extends Application implements ConnectionListener {
    ...
    public Message handle(Message msg, DTNHost host) {
        ...
        if (msg.getTo()==host && type.equalsIgnoreCase("ping")) {
            String id = "pong" + SimClock.getIntTime() + "-" + host.getAddress() + "->" + msg.getFrom().getAddress();
            DTNHost sourceHost = msg.getFrom();
            int sourceHostAddress = sourceHost.getAddress();
            ...
            super.sendEventToListeners("GotPing", sourceHostAddress, host);
            super.sendEventToListeners("SentPong", sourceHostAddress, host);
            return null; // to stop forwarding
        }
        // Received a pong reply
        else if (msg.getTo()==host && type.equalsIgnoreCase("pong")) {
            DTNHost sourceHost = msg.getFrom();
            int sourceHostAddress = sourceHost.getAddress();
            // Send event to listeners
            super.sendEventToListeners("GotPong", sourceHostAddress, host);
            return null; // to stop forwarding
        }
        else
            super.sendEventToListeners("Passing by", msg, host);
     ...
     * @return host
     */
    private DTNHost randomHost(int sourceAddress) {
        int destaddr = 0;
        if (destMax == destMin) {
            destaddr = destMin;
        }
    ...
    public void update(DTNHost host) {
        ...
        if (curTime - this.lastPing >= this.interval) {
            ...
            DTNHost destinationHost = randomHost(host.getAddress());
            Message m = new Message(host, destinationHost, "ping" +
                    SimClock.getIntTime() + "-" + host.getAddress() + "->" + destinationHost.getAddress(),
                    getPingSize());
            ...
            // Call listeners
            super.sendEventToListeners("SentPing", destinationHost.getAddress(), host);
