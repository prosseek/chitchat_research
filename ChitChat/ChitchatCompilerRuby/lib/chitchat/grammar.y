...
rule
  ...
  Call:
    # method(1, 2, 3)
    # or just variable name
    Variable Arguments          { result = CallNode.new(nil, val[0], val[1]) }
  ...
  SuperClass:
      /* nothing */               { result = nil }
    | '<' CONSTANT                { result = val[1] }

  Class:
   CLASS CONSTANT SuperClass Terminator Expressions END { result = ClassNode.new(val[1], val[4], val[2]) }
  ;
