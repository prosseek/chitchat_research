...
import conversion.BitUtilities._
import core.{Keys, Tables}

import scala.collection.BitSet
import scala.collection.mutable.{Map => MMap, BitSet =>  MBitSet}
 ...
 */
class BloomFilterCascade(dictionary:Map[String, Any], r:Int, m:List[Int], k:List[Int], seed:Int=0) {
  val keys = MMap[Int, Keys]() // Keys()
  val tables = MMap[Int, Tables]() // Tables(m=m, keys=k)
  val bitSetDictionary = BloomFilterCascade.data2bitArray(dictionary, r)
  ...
  def getAsValue(key:String, r:Int) : Int = {
    ...
    if (r == 8)
      ...
      bitSetToInt(value.get.asInstanceOf[BitSet])
    else
